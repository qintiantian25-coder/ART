...
def main(gt_dir, output_dir, input_dir=None, test_mask_csv=None, save_dir='results/blind_eval', crop_border=0):
    ...
    for img_name in out_imgs:
        ...
        if gt_path and os.path.exists(out_path):
            ...
            if gt_img is not None and out_img is not None:
                ...
                # full-frame metrics
                psnr_v = calculate_psnr(gt_img, out_img, crop_border, input_order='HWC')
                ssim_v = calculate_ssim(gt_img, out_img, crop_border, input_order='HWC')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', required=True, help='Output directory to evaluate')
    parser.add_argument('--input_dir', default=None, help='Optional input (blur) directory for input blind metrics')
    parser.add_argument('--test_mask_csv', default=None, help='CSV file with blind coords')
    parser.add_argument('--save_dir', default='results/blind_eval', help='Save directory for CSV')
    parser.add_argument('--crop_border', type=int, default=0, help='Crop border for PSNR/SSIM')
    args = parser.parse_args()

    main(args.gt_dir, args.output_dir, args.input_dir, args.test_mask_csv, args.save_dir, args.crop_border)
